...
export default function Activity({ data }: ActivityProps) {
    ...
    const dataGroupedByTime = useMemo(() => {
        ...
        const baseline = Date.parse("2025-09-30T00:00:00");

        const n = selectedTime === "Day" ? 1 :
                    selectedTime === "Week" ? 7 :
                    30;
                            ...
                            <div className={`flex-1 grid ${selectedTime === "Week" ? "grid-cols-7" : "grid-cols-12"} gap-x-2`}>
                                {times.find((t) => t.name === selectedTime)?.values.map((val, index) => {
                                    ...
                                    const avgValue =
                                        group.length > 0
                                        ? (
                                            group.reduce((sum, r) => sum + (typeof r.value === "number" ? r.value : 0), 0) /
                                            group.length
                                            ).toFixed(1)
                                        : null;
                                        ...
                                        <div key={index} className="flex flex-col items-center gap-y-1.5">
                                            ...
                                            >
                                                <div
                                                    ...
                                                    style={{
                                                        backgroundColor: group.length > 0 ? "rgba(73,130,255,0.8)" : "",
                                                        height: heightPx,
